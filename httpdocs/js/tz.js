...
/**
 * Function to get TimeZone for user
 * 
 * @return {string}
 */
 ...
 * correctly and not repeated
 * 
 * @param {string} tzString 
 * 
 * @return {string}
 */
function getTzFromString(tzString) {
    return [...new Set(tzString.replace(/\s/gm, "").split(","))].filter(el => tzList.indexOf(el)!=-1 || el=="user").join(",");
}

/**
 * Function to check if there is a time zone into string with several
 * time zones separated by comma.
 * 
 * @param {string} tzString string with several time zones
 * @param {string} tzSearch time zone to search in tzString
 * 
 * @return {boolean} true if exists
 */
function checkIfTzExistsInString(tzString, tzSearch) {
    return getTzFromString(tzString).split(",").some(el => el==tzSearch);
}

/**
 * Function to check if tz exists in array
 * 
 * @param {string} tz
 * 
 * @return {boolean}
 */
function checkIfTzExists(tz) {
    return tzList.some(el => el == tz);
}
 ...
 * 
 * @return {object|false} date or false
 */
const getDateForTimezone = (timezone, date) => {
    try {
...
/**
 * Function to return an object Date (now) for a timezone date.
 * 
 * @param {string} timezone 
 * 
 * @return {object|false} Return object Date of false it's timezone
 *                        doesn't exist
 */
